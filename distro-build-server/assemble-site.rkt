...
(module+ main
  ...
  (assemble-site #:config config-file
                 ...
                 #:input-dir (build-path 'same)
                 #:pkgs-all-file "build/pkgs.rktd"
                 #:missing-pkg-ok? #f
                 #:default-dist-base default-dist-base
                 #:default-pkgs default-pkgs))

(define (assemble-site #:config config-file
                       #:config-mode [config-mode #f]
                       #:input-dir [input-dir (build-path "compiled" "repackage")]
                       #:site-dest [site-dest #f]
                       #:default-dist-base [default-dist-base "racket"]
                       #:default-pkgs [default-pkgs '()]
                       #:pkgs-all-file [pkgs-all-file "compiled/repackage/build/built/catalog/pkgs-all"]
                       #:missing-pkg-ok? [missing-pkg-ok? #t])
  (define build-dir (build-path input-dir "build"))
  ...
  (define pkg-info-file (or pkgs-all-file
                            (build-path build-dir "pkgs.rktd")))
  ...
  (define (copy dir [build-dir build-dir] #:optional? [optional? #f])
    (define src (build-path build-dir dir))
  ...
  (unless (and (zero? (hash-count post-processes))
               (file-exists? installers-file-path))
  ...
  (copy "stamp.txt" #:optional? #t)
  (copy (build-path "origin" "collects.tgz") #:optional? #t)
